...
void loop() {
  ...
  if (condition) {
    ...
    if (checkTime-startTime>=1000){
       DrawCircle(row,col,10,sz, YELLOW_4); // row,col,color,wait,size(1=small,4=large);
       startTime = checkTime;
    }
  }
}
...
void DrawCircle(uint16_t row, uint16_t col, uint8_t wait, uint16_t condition, uint32_t colr) {
  ...
  for (int b = 0; b <= 100; b++) {
    if (b == 100){
      if (condition >= 4) {addressShape(xx4,yy4,row,col,ARRAY_LEN(xx4),strip.Color(0,0,0));}
      if (condition >= 3) {addressShape(xx3,yy3,row,col,ARRAY_LEN(xx3),strip.Color(0,0,0));}
      if (condition >= 2) {addressShape(xx2,yy2,row,col,ARRAY_LEN(xx2),strip.Color(0,0,0));}
      if (condition >= 1) {addressShape(xx1,yy1,row,col,ARRAY_LEN(xx1),strip.Color(0,0,0));}
    }
    if (b!= 100){
